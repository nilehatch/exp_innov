...
Two results bear on this book. **Firms holding identical representations underperform**, so there is a premium to seeing the market differently from your rivals. Matching their view is the losing move, not the safe one. And a representation that combines awareness of rivals with **flexibility** — keeping your options open until the uncertainty is resolved — largely insures its holder against the rival's thinking. That is a formal description of what this method builds.
...
The field evidence exists and points the same way, from a different direction. In a randomized controlled trial, entrepreneurs trained to treat their beliefs as theories and to test them explicitly made better decisions than a control group, pivoted more precisely, and abandoned failing ideas sooner [@camuffoScientificApproachEntrepreneurial2020]. That study is about the *posture* rather than about this particular procedure, which is the right kind of support to have: it says the discipline pays, and leaves the question of which method to run entirely open.
