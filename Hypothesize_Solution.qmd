...
All five run the same reversal, and it is the part that makes them work rather than merely break things: you apply the template first, describe the strange object it produces, and only then ask what it is good for. The full procedure, with the closed-world rule that keeps the results buildable, is in [Solution Guides](toolkit/Solution_Guides.qmd#sec-sit-guide).
