...
Unlike the other two diamonds, Diamond 3 never had guides: its methods were written into the chapters and stayed there. So this page is **extracted from** the chapters rather than written beside them, and every entry links back to the section it came from. All three phases now carry their procedures. The scoring matrix is deliberately absent while its place in the method is still being settled, and [The Method Layer](../method-layer.qmd) still stops at the end of Diamond 2.
...
Chapter: [Deliberately Bad Ideas](../Ideate_Solutions.qmd#sec-bad-ideas). Run it as one round inside brainwriting, or as a five-minute warm-up before the first round.

1. **State the target plainly.** The worst thing we could build for these people. Infeasible counts; something that makes the pain *worse* counts double.
...
The formal answer is **conjoint analysis**, also called a discrete choice experiment. You construct product profiles that vary systematically across the attributes you care about, ask people to choose or rate among them, and estimate a model in which the outcome is preference and each attribute contributes a coefficient. Those coefficients are the weights. It is a real and well-established method, and it is how the numbers in a professionally built scoring matrix are actually produced.

Two things stop it working at this point in an expedition, and both are worth understanding rather than taking on trust.

**You cannot estimate seven weights from three concepts.** The method needs profiles designed so that attributes vary independently of one another, and enough of them that each coefficient is identified. What you hold after screening is a handful of survivors differing in tangled ways. There is nothing to regress.
...
The scoring matrix procedure is not in this guide, deliberately. Its place in the method is still being settled.
